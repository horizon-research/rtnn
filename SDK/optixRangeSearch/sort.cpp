...
void initBatches(WhittedState& state) {
  ...
  state.d_r2q_map = new unsigned int*[B];
                ...
                )
{
    ...
    for (int i = 0; i < state.numOfBatches; i++) {
      ...
      // but after this point state.params.queries isn't meaningful anymore
      // because all queries will be pointed by state.d_actQs[i] and
      // state.params.queries will be set accordingly before launch.
